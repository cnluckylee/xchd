  $(function(){
	...
	var pid;
		...
		cjstart_btn='';
		$('.lotteryUserNum').html('<img src="cj_plug/images/loading.gif" />');
		$('.winUserNum').html('<img src="cj_plug/images/loading.gif" />');
		  $(".lotteryImg").attr("src","cj_plug/images/pair-default.jpg");
		  $(".lotteryName").html("... ...");
		   cj_per= new Array(); 
	...
	$('#cj_resetBtn').click(function(){
       if(confirm("确定重置摇奖池吗？")){
				...
				clearInterval(cj_gogo);
				cjstart_btn.children("span").text("正在准备数据…");
			  $.post("cj_plug/cj_data.php?action=reset",{},function(msg){
				  if(msg==2){
					cj_jadge = 1;
					cj_id = 0;
					$(".winUserList").empty();
